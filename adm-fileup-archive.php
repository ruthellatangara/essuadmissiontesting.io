...
<body>
    ...
    if (isset($_SESSION['email'])) {
        $apprefno=$_GET['id'];
        $sql = mysqli_query($con, "SELECT idPic,rCard,gMoral,sy FROM student_archive where apprefno='$apprefno'");
        $uploads = mysqli_fetch_array($sql);
        $sy = $uploads['sy'];
        echo '
            <a href="adm-getform-archive.php? id='.$apprefno.'" class=" btn btn-primary btn-sm">&#8592;</a>
        ';
        ?>
