...
include 'connect.php';
        ...
        $query = "SELECT apprefno,CONCAT (fname, ' ', mname, ' ', lname) AS name,email,contact,tsite,sy from account_tab WHERE apprefno='".$_POST['apprefno']."'";
        ...
        $my_sql="Select * from account_tab where tsched='".$_POST["tsched"]."'";
        $my_run=mysqli_query($con,$my_sql);
    
        if(mysqli_num_rows($my_run) < 100){
            $relresult = $_POST["d_result"];
            $sched = $_POST["tsched"];
            $tsched = date_create($_POST["tsched"]);
        ...
        $update = mysqli_query($con,"UPDATE account_tab set `tsched` = '$sched', relresult='$relresult', stat1=1, stat2=1 WHERE apprefno='".$_POST['apprefno']."'");
        ...
        $pdf->Cell(30,4,'APP_REF NO.',0,0,'L');
        $pdf->Cell(80,4,''.$apprefno,0,1,'L');
        $pdf->SetFont('helvetica','',9);
        $pdf->Cell(35,4,'Name of Examinee:',0,0,'L');
        $pdf->Cell(80,4,''.$name,0,1,'L');
        $pdf->SetFont('helvetica','',9);
        $pdf->Cell(35,4,'Date of Examination:',0,0,'L');
        $pdf->Cell(45,4,'AY '.$acadYear.' '.$sem.' '.$formatd,0,1,'L');
        $pdf->Cell(20,4,'Testing Site:',0,0,'L');
        $pdf->Cell(35,4,''.$tsite,0,0,'L');
        $pdf->Ln(5);
        $pdf->Cell(65,4,'Date of Release of Examination Result: ',0,0,'L');
        $pdf->Cell(10,4,''.$relresult,0,1,'L');
        $pdf->Ln(1);

        $pdf->SetFont('helvetica','I',9);
        $pdf->Cell(50,4,'Reminder: Bring the following:',0,1,'L');
        $pdf->Cell(50,4,'     1.   Examination Permit',0,1,'L');
        $pdf->Cell(50,4,'     2.   School ID',0,1,'L');
        $pdf->Cell(50,4,'     3.   Pencil No.2 sharpener and eraser',0,1,'L');
        $pdf->Cell(50,4,'     4.   Blue ballpen',0,1,'L');
        $pdf->Cell(50,4,'     5.   Snacks (optional)',0,1,'L');
        ...
        $pdfFile = $pdf->Output('essu-admission-permit.pdf','S');

        $body = "Hi,";
        $body .='<p>Your Examination Permit for Eastern Samar State University Admission Exam Application is already available! Kindly check your account download the attached file below. Thank you.</p>
            <div class= "container mt-3 mb-4">
                    <br>
                    <div class="col-md-12">
                        APP_REF NO.: '.$row['apprefno'].'
                        <br>
                        Name of Examinee: '.$row['name'].'
                        <br>
                        Date of Examination: AY '.$acadYear.' '.$sem.' '.$formatd.'
                        <br>
                        Testing Center: '.$row['tsite'].'
                        <br>
                        Date of Release of Examination Result: 
                    </div>
            </div>
        ';
        $subject = "UNIVERSITY ENTRANCE EXAMINATION PERMIT";
        ...
        $mail->addStringAttachment($pdfFile,'essu-admission-permit.pdf');
        ...
        $mail->AddAddress($email_to);
            if (!$mail->Send()) {
                ...
                //header('location:adm-result.php');
            }
    }
    else{
        mysqli_query($con,"UPDATE tsched_tab SET stat=1 WHERE sched='".$_POST["tsched"]."'");
        echo "Failed. Sorry you have reached the maximum limit of permit issued for this schedule.";
