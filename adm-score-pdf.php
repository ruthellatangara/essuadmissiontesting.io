...
if (isset($_SESSION['email'])) {
    ...
    $query = "SELECT account_tab.apprefno,account_tab.fname, account_tab.mname, account_tab.lname,account_tab.email,account_tab.contact,DATE_FORMAT(FROM_DAYS(DATEDIFF(now(),dob)),'%Y')+0 as age,account_tab.sex,account_tab.tsched, account_tab.tsite,account_tab.lschool,account_tab.campus,account_tab.sy,account_tab.fchoice,account_tab.schoice,account_tab.tchoice,result_tab.scaledscore,result_tab.sai,result_tab.perrank,result_tab.stanine,result_tab.vcscore,result_tab.vrscore,result_tab.frscore,result_tab.qrscore from account_tab INNER JOIN result_tab on account_tab.apprefno = result_tab.stuID where result_tab.stuID='$tpn'";

    //$query="SELECT * from result_tab where stuID='$tpn'";
    ...
    /*$fetch = mysqli_query($con, "SELECT acadYear, sem FROM tsched_tab where sched='".$tdate."'");
    $tsched = date_create($tdate);
    $formatd = date_format($tsched, "F d, Y h:i a");
    $frow=mysqli_fetch_assoc($fetch);
    $acadYear = $frow['acadYear'];
    $sem = $frow['sem'];*/

    $fetch = mysqli_query($con, "SELECT sched, code FROM tsched_tab where sched='".$tdate."'");
    $tsched = date_create($tdate);
    $formatd = date_format($tsched, "F d, Y h:i a");
    $frow=mysqli_fetch_assoc($fetch);
    $code = $frow['code'];
